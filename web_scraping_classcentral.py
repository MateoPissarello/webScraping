...
def get_nav(soup):
    ...
    nav_bar = soup.find(
        "nav",
        attrs={
            "class": "cmpt-nav row nowrap vert-align-middle absolute width-100 padding-horz-medium border-box"
        },
    )
    login_singup = nav_bar.find(
        "div", attrs={"class": "hidden xlarge-up-inline-block"}
    ).find_all("a")
...
def get_popularCourses(soup):
    list_elements = []
    popular_courses = soup.find(
        "div",
        attrs={
            "class": "width-100 medium-up-width-2-3 border-box medium-up-padding-left-large horz-align-right"
        },
    )
    subject_university_learn_more = popular_courses.find_all(
        "a", attrs={"class": "link-gray-underline"}
    )
    for i in subject_university_learn_more:
        text = i.text
        url = i["href"]
        if "www.classcentral.com" in str(url):
            url = url
        else:
            url = "http://classcentral.com" + url
        data = {
        ...
        list_elements.append(data)
    li_popular_courses = popular_courses.find(
        "ul", attrs={"id": "home-subjects"}
    ).find_all("li")
...
def get_footer(soup):
    ...
    footer_div = soup.find(
        "div",
        attrs={
            "class": "width-page large-down-padding-horz-medium padding-vert-large border-box"
        },
    )
...
def get_all_collections(soup):
    ...
    collections = soup.find(
        "ul", attrs={"class": "list-no-style row wrap margin-bottom-xxlarge"}
    ).find_all("li")
...
def get_all_buttons(soup):
    list_elements = []
    buttonsPurple = soup.find_all(
        "a", attrs={"class": "btn-gradient-purple scale-on-hover"}
    )
    orangeButtons = soup.find(
        "a", attrs={"class": "btn-gradient-orange scale-on-hover"}
    )
...
def get_data_courses_guides(soup):
    list_elements = []
    courses = soup.find(
        "ul", attrs={"class": "margin-bottom-xlarge list-no-style"}
    ).find_all("li")
    for course in courses:
        img = (
            course.find(
                "div",
                attrs={"class": "width-100 medium-up-width-1-3 horz-align-left row"},
            )
            .find("a")
            .find("img")["src"]
        )
        link = course.find(
            "div", attrs={"class": "width-100 medium-up-width-1-3 horz-align-left row"}
        ).find("a")["href"]
        author = (
            course.find(
                "div",
                attrs={
                    "class": "border-box width-100 medium-up-width-2-3 medium-up-padding-left-medium"
                },
            )
            .find("p")
            .find("strong")
        ...
        text = course.find(
            "a", attrs={"class": "head-3 medium-up-head-2 color-charcoal"}
        ).text
        data = {
            "img_course": img,
            "link": link,
...
def get_data_article(soup):
    ...
    for article in articles:
        arti = article.find(
            "a", attrs={"class": "head-3 weight-semi line-tight color-charcoal"}
        )
        ...
        data = {
            ...
            "publish": author.find(
                "span", attrs={"class": "text-2 inline-block color-gray inline-block"}
            ).text,
        }
        list_elements.append(data)
    return list_elements
...
def get_full_data(soup, xpath, type=None, index=None):
    ...
    if type == "midpage":
        if index is None:
            ...
            elements = (
                soup.find_all("li", attrs={"class": xpath})[index]
                .find("ul")
                .find_all("li")
            )
        for ele in elements:
            ...
            data = {
                ...
                "name": a.find(
                    "strong", attrs={"class": "margin-left-xsmall fill-space"}
                ).text,
                "img": a.find("img")["src"],
            }
            list_elements.append(data)
    ...
    if type == "imgs":
        ...
        for i in imgs:
            data = {"svg": i.find("svg"), "img_src": i.find("svg").find_all("path")}
            list_elements.append(data)
...
def get_data_all(soup):
    ...
    rankings = soup.find(
        "ul",
        attrs={
            "class": "max-850 width-centered list-no-style row large-up-nowrap margin-bottom-xxlarge"
        },
    )
    universities = get_full_data(
        down,
        "border-box width-100 large-up-width-3-7 padding-horz-small relative",
        "midpage",
    )
    providers = get_full_data(
        down,
        "medium-down-hidden decor-grid-line border-box width-100 large-up-width-2-7 padding-horz-small large-up-padding-horz-large relative",
        "midpage",
        0,
    )
    institutions = get_full_data(
        down,
        "medium-down-hidden decor-grid-line border-box width-100 large-up-width-2-7 padding-horz-small large-up-padding-horz-large relative",
        "midpage",
        1,
    ...
    ranks = get_full_data(
        rankings,
        "width-100 border-box padding-small large-up-width-1-2 border-box",
        "rankings",
    )
